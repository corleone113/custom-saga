...
function run(saga, { // 第二个参数改为选项对象，方便扩展
    next: endTask,
    fromFork,
} = {}) {
    ...
    function next(nextValue) { // saga执行器，要引用很多局部变量，所以定义在内部
        ++iteractionCount;
        if (iteractionCount >= 1000) // 迭代次数阈值为1000，防止短时间内连续执行多次
            return;
        const {
            done,
            value: effect
        } = it.next(nextValue); // 将yield表达式结果作为内部yield返回值
        if (!done) {
            if (!effect) { // 非对象/函数则直接进行下一步且在saga内部返回该值
                next(effect);
            } else if (typeof effect[Symbol.iterator] == 'function') { // 可能是新的saga任务
                run(effect, {
                    next,
                });
            } else if (typeof effect.then == 'function') { // 是promise则等待其结束再进行后续
                effect.then(next);
            } else if (!effect.type) { // 无效的effect也直接进行下一步且该值作为内部yield返回值
                next(effect);
            } else { // 到这里说明是有效的effect，然后根据effect进行相应的操作。
...
function handleEffect(effect, next, currentTask) {
    ...
    switch (effect.type) {
        ...
        case 'FORK': // 开启一个新的saga任务
            ...
            const gen = eval(transform(task.toString(), { // 通过finalTaskPlugin将任务中finally语句转化为一个generator回调在CANCELLED Effect中调用，目前CANCELLED EFFECT的实现方式有问题，后续完整阅读了redux-saga源码后再修改。
                plugins: [
                    finalTaskPlugin,
                ]
            }).code);
            ...
            run(newTask, { // 传入saga任务迭代器，通过run启动新的saga任务
        ...
        case 'CALL': // 执行一个函数并获取其结果作为内部yield返回值
            ...
            ({
                fn,
                context
            } = getCallbackFromFirst(first)); // 获取有效的回调和为其绑定的this参数
            const ret = fn.call(context, ...args); // 执行回调并保存其结果
            if (typeof ret[Symbol.iterator] === 'function') {
                run(ret, { // 是genenrator函数时则开启新saga任务，等待其执行完后再进行后续并将其return结果作为内部yield返回值。
                    next
                });
            } else if (typeof ret.then === 'function') { // thenable或promise则等待其结束时进行下一步，异步结果作为内部yield返回值。
                ret.then(next);
            } else {
        ...
        case 'CPS': // 调用nodejs风格的函数
            ...
            ({
                fn,
                context
            } = getCallbackFromFirst(first,)); // 这几步和上面CALL Effect中一样
            const {
                length,
            } = args;
            const cb = args[length - 1]; // 获取传入回调的最后一个参数，这个参数应该是一个回调
            if (typeof cb !== 'function') { // 最后一个参数不为回调则主动提供一个回调作为最后一个参数。
                const cb = (err, msg) => {
                    try {
        ...
        case 'RACE': // 批量处理多个effect，但只要有一个effect执行完就以其结果作为内部yield返回值并中断其它effect的处理。
            ...
            if (typeof effects === 'object') {
                ...
                const raceCallback = (k, isArr) => (v) => { // raceCallback是RACE Effect开启的每个effect处理结束时的回调
                ...
                if (Array.isArray(effects)) { // 批量处理的effect保存在数组中
                    for (let i = 0; i < effects.length; ++i) {
                        handleEffect(effects[i], raceCallback(i, true));
                    }
                } else { // 批量处理的effect保存在对象中
        ...
        case 'SELECT': // 获取当前state并用指定回调处理。
            ...
            break;
        default: // 无法识别的effect则直接开启下一步并以该effect作为内部yield返回值。
            next(effect);
            break;
    }
}
...
function createChannel() {
    ...
    const subscribe = (actionType, callback) => { // 订阅actionType对应的回调
...
function getCallbackFromFirst(first) { // 用于尝试从first获取有效的回调和为其绑定的this参数。
    let fn, context;
    ...
    return { // 将finally语句转化为回调函数
        visitor: {
            FunctionDeclaration(path) {
                ...
                path.traverse({
                    TryStatement(tryPath) { // 进入try语句
                        const {
                            finalizer
                        } = tryPath.node
                        if (finalizer && finalizer.body.length > 0) { // 具有finally语句且语句中含有代码
